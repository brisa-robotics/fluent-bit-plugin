...
static int set_dummy_timestamp(msgpack_packer *mp_pck, struct flb_dummy2 *ctx) {
  struct flb_time t;
...
static int gen_msg(struct flb_input_instance *ins, void *in_context,
  ...
  struct flb_dummy2 *ctx = in_context;
...
static int config_destroy(struct flb_dummy2 *ctx) {
...
static int configure(struct flb_dummy2 *ctx, struct flb_input_instance *in,
                     struct timespec *tm) {
  struct flb_time dummy_time;
  const char *msg;
  ...
  if (ret == 0) {
    ...
    ret = flb_pack_json(ctx->dummy_message, ctx->dummy_message_len,
...
static int in_dummy_init(struct flb_input_instance *in,
  ...
  int ret = -1;
  struct flb_dummy2 *ctx = NULL;
  struct timespec tm;

  /* Allocate space for the configuration */
  ctx = flb_malloc(sizeof(struct flb_dummy2));
  if (ctx == NULL) {
    return -1;
  }
  ctx->ins = in;
  ...
  ret = flb_input_set_collector_time(in, in_dummy_collect, tm.tv_sec,
                                     tm.tv_nsec, config);
  if (ret < 0) {
...
static struct flb_config_map config_map[] = {
    {FLB_CONFIG_MAP_INT, "samples", "0", 0, FLB_TRUE,
     offsetof(struct flb_dummy2, samples),
     "set a number of times to generate event."},
    {FLB_CONFIG_MAP_STR, "dummy", DEFAULT_DUMMY_MESSAGE, 0, FLB_FALSE, 0,
     "set the sample record to be generated. It should be a JSON object."},
    {FLB_CONFIG_MAP_INT, "rate", "1", 0, FLB_TRUE,
     offsetof(struct flb_dummy2, rate), "set a number of events per second."},
    {FLB_CONFIG_MAP_INT, "copies", "1", 0, FLB_TRUE,
     offsetof(struct flb_dummy2, copies),
     "set the number of copies to generate per collectd."},
    {FLB_CONFIG_MAP_INT, "start_time_sec", "-1", 0, FLB_TRUE,
     offsetof(struct flb_dummy2, start_time_sec),
     "set a dummy base timestamp in seconds."},
    {FLB_CONFIG_MAP_INT, "start_time_nsec", "-1", 0, FLB_TRUE,
     offsetof(struct flb_dummy2, start_time_nsec),
     "set a dummy base timestamp in nanoseconds."},
    {FLB_CONFIG_MAP_BOOL, "fixed_timestamp", "off", 0, FLB_TRUE,
     offsetof(struct flb_dummy2, fixed_timestamp),
     "used a fixed timestamp, allows the message to pre-generated once."},
    {0}};

struct flb_input_plugin in_dummy2_plugin = {.name = "dummy2",
                                            .description =
                                                "Generate dummy data",
